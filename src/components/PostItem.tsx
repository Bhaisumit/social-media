      ...
      <Link to={`/post/${post.id}`} className="block relative z-10">
        <div className="w-80 h-76 bg-[rgb(24,27,32)] border border-[rgb(84,90,106)] rounded-[20px] text-white flex flex-col p-5 overflow-hidden transition-colors duration-300 group-hover:bg-gray-800">
          <div className="flex items-center space-x-2">
            {post.avatar_url ? (
              <img className="w-[35px] h-[35px] rounded-full object-cover" src={post.avatar_url} alt="user Avatar" />
            ) : (
          ...
          <span className="cursor-pointer h-10 w-[50px] px-1 flex items-center justify-center font-extrabold rounded-lg">   ❤️
